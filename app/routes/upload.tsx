...
const upload = () => {
  ...
  const handleAnalyze = async ({ companyName, jobTitle, jobDescription, file }:
    ...
    setStatusText("Preparing the final results...");
    ...
    if(!feedback) {
      setIsProcessing(false);
      setStatusText("Failed to analyse resume");
      return;
    }
    ...
    await kv.set(`resume:${uuid}`, JSON.stringify(data));
    setIsProcessing(false);
    setStatusText("Your resume is ready!Redirecting to a new page");
    console.log(data);
  ...
  const handleSubmit = (e: FormEvent<HTMLFormElement>) => {
    ...
    <section className="main-section">
      <div className="page-heading py-15">
        <h1>Smart feedback that assures you the job.</h1>
        <h2>Resumio is the best ai reviewer in the market.</h2>
        {isProcessing ? (
            <>
              <h2>{statusText}</h2>
            ...
            <h2>Drop your resume for a full ATS score and review</h2>
        )}
        {!isProcessing && (
